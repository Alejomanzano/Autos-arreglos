...
void ingresar(void){
    if(CantProductos < MaxInventario){
        ...
        FILE *fp = fopen("autos.txt", "a"); 
        if (fp!= NULL) {
            fprintf(fp, "%s;%s;%s;%i;%.2f\n", productos[CantProductos].marca, productos[CantProductos].modelo, productos[CantProductos].tipo, productos[CantProductos].year, productos[CantProductos].precio);
            fclose(fp);
        } else {
...
void editar(int cantidadproductos){
    if(cantidadproductos > 0){
        ...
        FILE *fp = fopen("autos.txt", "w"); 
        if (fp!= NULL) {
            for (int i = 0; i < CantProductos; i++) {
                fprintf(fp, "%s;%s;%s;%i;%.2f\n", productos[i].marca, productos[i].modelo, productos[i].tipo, productos[i].year, productos[i].precio);
            }
            fclose(fp);
        }    
    }
